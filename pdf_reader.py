...
def get_merger_frame(parent):
    ...
    # UI layout
    Label(frame, text="Source Folder").grid(row=0, column=0, sticky="w", padx=10, pady=5)
    Entry(frame, textvariable=source_folder, width=50).grid(row=0, column=1, padx=5, pady=5)
    Button(frame, text="Browse", command=lambda: browse_folder(source_folder)).grid(row=0, column=2, padx=5, pady=5)

    Label(frame, text="Attachment 1").grid(row=1, column=0, sticky="w", padx=10, pady=5)
    Entry(frame, textvariable=attachment1, width=50).grid(row=1, column=1, padx=5, pady=5)
    Button(frame, text="Browse", command=lambda: browse_file(attachment1)).grid(row=1, column=2, padx=5, pady=5)

    Label(frame, text="Attachment 2").grid(row=2, column=0, sticky="w", padx=10, pady=5)
    Entry(frame, textvariable=attachment2, width=50).grid(row=2, column=1, padx=5, pady=5)
    Button(frame, text="Browse", command=lambda: browse_file(attachment2)).grid(row=2, column=2, padx=5, pady=5)

    Label(frame, text="Output Folder").grid(row=3, column=0, sticky="w", padx=10, pady=5)
    Entry(frame, textvariable=output_folder, width=50).grid(row=3, column=1, padx=5, pady=5)
    Button(frame, text="Browse", command=lambda: browse_folder(output_folder)).grid(row=3, column=2, padx=5, pady=5)

    Button(frame, text="Merge PDFs", command=merge_pdfs, bg="green", fg="white").grid(
        row=4, column=0, columnspan=3, pady=20
    )

    Label(frame, text="Merge Log", bg="lightgrey", font=("Arial", 10, "bold")).grid(
